...
**Key Endpoints (Initial Implementation):**
- `POST /v0/authorize/machine-token` - Exchange machine token for session
- `GET /v1/sites/{site_id}` - Get site details
- `GET /v1/sites/{site_id}/environments` - List environments
- `GET /v1/sites/{site_id}/environments/{env}` - Get environment details
- `PUT /v1/sites/{site_id}/environments/{env}` - Update environment (SFTP/Git mode toggle)
- `POST /v1/sites/{site_id}/environments/{env}/backups` - Create backup
- `GET /v1/sites/{site_id}/environments/{env}/backups` - List backups
- `POST /v1/sites/{site_id}/environments/{env}/code` - Deploy code
- `GET /v1/workflows/{workflow_id}` - Check workflow status
- Domain management endpoints (TBD - to be explored for multisite PoC)
- Upstream update endpoints (TBD - to be discovered from API docs)
